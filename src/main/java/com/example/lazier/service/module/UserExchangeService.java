...
public class UserExchangeService {
	...
	public void add(String paramId) {
        LazierUser lazierUser = memberRepository.findByUserId(Long.parseLong(paramId))
	...
	public List<UserAllExchangeDto> getExchange(HttpServletRequest request) {
		...
		for (int i = 0; i < 10; i++) {
			...
			Exchange exchange = exchangeRepository.findByCurrencyNameOrderByUpdateAtDescCountryNameDesc(
					currencyName)
